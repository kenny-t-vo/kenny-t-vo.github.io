#!/usr/bin/env bash
# Render a PDF into page images for one directory.
#   ./build.sh path/to/book.pdf <dir>
# e.g. ./build.sh "~/portfolio.pdf" photography
#      ./build.sh "~/cv.pdf" cv
#
# photography and work get three lossy tiers and handle both export shapes:
#   portrait pages  -> paired as cover, 2-3, 4-5 ... last alone
#   landscape 2-ups -> already spreads; split into leaves, paired 1-2, 3-4 ...
#
# cv gets one lossless tier, sharper and smaller than lossy for type, and no
# zoom or index.
#
# Requires: poppler (pdftoppm), webp (cwebp), python3 with Pillow, and pypdf
# for links.
#   brew install poppler webp && pip3 install pillow pypdf
set -euo pipefail
...
if [ "$BOOK" = "cv" ]; then
  ...
  # aspect comes from the pdf page box; this Pillow cannot decode webp
  read -r CV_PTS_W CV_PTS_H < <(pdfinfo "$PDF" | awk '/^Page size:/ {print $3, $5}')
  PYTHONPATH="$ROOT" DEST="$DEST" PDF="$PDF" PAGES="$n" WORK="$WORK" \
    PTS_W="$CV_PTS_W" PTS_H="$CV_PTS_H" python3 - <<'CVPY'
import glob, json, os

dest, pdf = os.environ["DEST"], os.environ["PDF"]
page_count = int(os.environ["PAGES"])
aspect = round(float(os.environ["PTS_W"]) / float(os.environ["PTS_H"]), 6)

# link fitting reads the png renders; this Pillow cannot decode webp
masters = sorted(glob.glob(f"{os.environ['WORK']}/p-*.png"))
def leaf(n):
    if not (1 <= n <= len(masters)): return None
    from PIL import Image
    return Image.open(masters[n - 1]).convert("RGB")
...
try:
    ...
    links = linkmap.fit_to_ink(links, leaf, f"{dest}/pages/link", "pages/link")
    print(f"   {sum(1 for v in links.values() for x in v if 'chip' in x)} highlight chip(s)")
except ImportError:
...
try:
    ...
    print(f"→ carried over {linkmap.count(links)} hyperlink(s)")
    # snap each rect to its ink and render the inverted highlight patch
    links = linkmap.fit_to_ink(links,
                               lambda n: pages[n - 1] if 1 <= n <= len(pages) else None,
                               f"{dest}/pages/link", "pages/link")
    print(f"   {sum(1 for v in links.values() for x in v if 'chip' in x)} highlight chip(s)")
except ImportError:
    print("⚠  pypdf not installed: hyperlinks not carried over (pip3 install pypdf)")
